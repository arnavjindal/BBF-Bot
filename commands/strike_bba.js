...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('strike_bba')
		.setDescription('Add a Strike for a BBa Player')
	...
	async execute(interaction) {
		...
		var path_ = `${path.join(__dirname, `../bba/strike/${targett.id}.json`)}`;
		...
		if (fs.existsSync(path_)) {
			...
			// Any changes in the format of objj would impact the object above and object in reset Command file.
			fs.writeFile(path_, JSON.stringify(objj), (err) => {
				if (err) throw err;
			});
			const exampleEmbed = {
